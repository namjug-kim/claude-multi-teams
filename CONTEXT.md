...
| session/response file | predictable path; framework globs `~/.claude-spare/projects/*/<uuid>.jsonl` (appears ~0.3s after first prompt) | rollout file at `$CODEX_HOME/sessions/YYYY/MM/DD/rollout-*.jsonl`; each codex pane gets a **per-agent `CODEX_HOME`** (`<state_dir>/codex-home/<agent_id>`, the real home mirrored by symlinks but with a private `sessions/`), so the max-mtime "newest rollout after spawn" scan is unambiguous even under many concurrent first-asks (see ADR-0008) | none tracked — `.pb` files don't persist for short sessions and are opaque even if they did; capture-pane is the only channel |
